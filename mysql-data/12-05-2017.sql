...
insert  into `error_code`(`code`,`details`) values (200,'completed'),(201,'not completed'),(202,'battle closed'),(203,'battle not found'),(204,'not enough coin to buy this lesson'),(205,'lesson was bought'),(206,'not enough money'),(207,'lesson wasn\'t bought'),(208,'inbox item not found'),(209,'insert and update progress success'),(210,'update progress success');
...
insert  into `lesson_unit_tracker`(`lst_id`,`user_id`,`lu_id`,`lst_progress`,`lst_time`) values (1,0,52,3,NULL);
...
/* Procedure structure for procedure `update_lesson_unit` */

/*!50003 DROP PROCEDURE IF EXISTS  `update_lesson_unit` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_lesson_unit`(
		in p_user_id varchar(50),
		in p_lu_id int(11),
		in p_progress int(11)
    )
BEGIN
	select count(*) from `lesson_unit_tracker` where `lesson_unit_tracker`.`user_id` = p_user_id and `lesson_unit_tracker`.`lu_id` = p_lu_id into @check;
	if (@check = 1) then
		UPDATE `silly_english`.`lesson_unit_tracker`
		SET`lst_progress` = p_progress
		WHERE `lesson_unit_tracker`.`lu_id`=p_lu_id and `lesson_unit_tracker`.`user_id` = p_user_id;
		select * from error_code where error_code.`code` = 209;
	else
		INSERT INTO `silly_english`.`lesson_unit_tracker`
            (`user_id`,
             `lu_id`,
             `lst_progress`)
		VALUES (p_user_id,
